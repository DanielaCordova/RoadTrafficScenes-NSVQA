...
class ClassifierCore(ABC):
    ...
    def read_dataframe(self, path: str, confidence: float = 0.5, min_bbox_area: int = 3731,
                       min_class_img_count: int = 0, pixel_dilation: int = 0, train_mode: bool = True):
        """
        ...
        :return: pd.DataFrame
        """
        df = pd.read_csv(path, usecols=['Make', 'image_id', "x1", "y1", "x2", "y2"])
        ...
        df["x1"] = pd.to_numeric(df["x1"], downcast="integer", errors='coerce')
        df["x2"] = pd.to_numeric(df["x2"], downcast="integer", errors='coerce')
        df["y1"] = pd.to_numeric(df["y1"], downcast="integer", errors='coerce')
        df["y2"] = pd.to_numeric(df["y2"], downcast="integer", errors='coerce')
        # Bbox image size in pixel area]
        df["area"] = ( df["x2"]- df["x1"]) * (df["y2"] - df["y1"])  # Format: xyxy
        df = df.loc[df["area"] >= min_bbox_area].reset_index(drop=True)
        ...
        df["x1"] = np.maximum(df["x1"] - pixel_dilation, 0)
        df["y1"] = np.maximum(df["y1"] - pixel_dilation, 0)
        df["x2"] = df["x2"]+ pixel_dilation
        df["y2"] = df["y2"] + pixel_dilation
        ...
        #df["x1"] , df["y1"] , df["x2"] , df["y2"] = df.apply(enforce_boundaries, axis=1)

        df["x2"]= df["x2"] - df["x1"]
        df["y2"] = df["y2"] - df["y1"]
        # Modify bounding boxes, originally xyxy
        #df['Bboxes'] = df['Bboxes'].apply(lambda x: [x[1], x[0], x[3] - x[1], x[2] - x[0]])  # rearranging for Tensorflow's preferred [y, x, y2-y1, x2-x1]
        # Modify bounding boxes, originally xyxy
        ...
        # Rearrange
        df = df[['Make', 'image_id', 'x1', 'y1', 'x2', 'y2']]

        df['Bboxes'] = list(zip(df['x1'], df['y1'], df['x2'], df['y2']))

        if train_mode:

            # Remove make-model combinations below minimum image count
            counts = df.groupby('Make')['image_id'].count().reset_index()
            excluded = counts[counts['image_id'] < min_class_img_count]['Make'].tolist()
            df = df.loc[~df['Make'].isin(excluded)]

            # Convert Make-Model to int, generate label mapping
            df['Make'] = df['Make'].astype('category')

            temp = dict(zip(df['Make'].drop_duplicates().cat.codes, df['Make'].cat.categories))
            ...
            df['Make'] = df['Make'].astype('category').cat.codes  # Converts to int



            # Convert Make-Model to onehot encodings
            dummies = pd.get_dummies(df['Make'], prefix='')
            dummies.columns = [i[1:] for i in dummies.columns]

            # Concat together
            df = pd.concat([df[['image_id', 'x1', 'y1', 'x2', 'y2', "Bboxes"]], dummies], axis=1)
        ...
        # Shuffle data
        df = df.sample(frac=self.config['sample'], random_state=100).reset_index(drop=True)

        # Create stratified random sample
        if self.config['sample']< 1.0:
            reverse_onehot = df.iloc[:, 6:].idxmax(axis=1).astype(int).reset_index()  # recover argmax
