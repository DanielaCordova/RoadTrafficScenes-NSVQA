...
if __name__ == '__main__':
    ...
    df = pd.read_csv('E:/TFM/Dataset1Car//make_model_database.csv')
    ...
    merged_df = df.groupby(['Make', 'Detailed Model','Model']).agg({'Category': ', '.join, 'Year': ', '.join}).reset_index()

    df.to_csv('E:/TFM/Dataset1Car/make_model_database_mod.csv', index=False)

    merged_df.to_csv('E:/TFM/Dataset1Car/make_model_database_modMerge.csv', index=False)